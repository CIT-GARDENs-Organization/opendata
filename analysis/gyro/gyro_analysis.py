...
センサは FSM300（BNO085系9軸IMU；加速度計・ジャイロ・地磁気センサをオンボードで融合し
Rotation Vector を出力する）．軌道上は自由落下で加速度計の重力基準が失われるが，地磁気センサが
地球磁場という絶対方向を与えるため，クォータニオンは地磁気を基準とする絶対姿勢になる（クォータ
ニオンがドリフトせず周期的なのはこの絶対基準の証拠）．鉛直に対する傾きだけが決まらない．回転
（スピン）はジャイロで直接測れる．本スクリプトは，(1)クォータニオン連続サンプルから回転速度を
求め，(2)独立な物理量であるSAP（太陽電池）電流の変調からスピン周期と軸を求めて突き合わせる．
...
def main():
    ...
    for s, g in df.groupby("session"):
        ...
        if len(w):
            # 瞬間回転軸（wx,wy,wz）/|w| の平均軸まわりのばらつき．
            # 差分クォータニオンが単一軸のスピンを追えているなら小さい．
            gg = g.dropna(subset=["wx", "wy", "wz", "wmag"])
            u = gg[["wx", "wy", "wz"]].to_numpy(float)
            u = u / np.linalg.norm(u, axis=1)[:, None]
            ax = u.mean(0)
            ax = ax / np.linalg.norm(ax)
            scatter = np.degrees(np.arccos(np.clip(u @ ax, -1, 1)))
            msg += (f" |w| median={w.median():.2f} deg/s"
                    f" [q25={w.quantile(.25):.2f}, q75={w.quantile(.75):.2f}]"
                    f" body-axis=({ax[0]:+.2f},{ax[1]:+.2f},{ax[2]:+.2f})"
                    f" scatter_median={np.median(scatter):.0f}deg")
        print(msg)
