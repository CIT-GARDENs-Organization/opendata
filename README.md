...
- `data/downlink/<sat>/`: 衛星からダウンリンクしたデータ．
  ...
  - `gyro/gyro.csv`: BOTANのジャイロミッション（IMU姿勢）のダウンリンク16進フレーム．復号は `data/derived/04_botan/gyro_attitude.csv`．
...
- `data/derived/<sat>/`: 解析で生み出した2次データ（表）．日次HK（`hk_daily.csv`：生HKの日別中央値），軌道高度（`orbit.csv`：TLEから算出），姿勢解析の日別指標（`beta_daily.csv`, `attitude_daily.csv`, `eclipse_events.csv`），APRS局一覧（`aprs_stations.csv`）．BOTANのみ地上局の日次受信件数（`downlink_daily.csv`），MSGミッションのデコード済みメッセージ（`aprs_messages.csv`），ジャイロの復元クォータニオン（`gyro_attitude.csv`）．号機間比較の表は `data/derived/00_compare/`．
- `analysis/`: 2次データを再現する解析スクリプト，one sub-folder per topic. See [analysis/README.md](analysis/README.md).
  ...
  - `aprs/`: `aprs_analysis.py`（APRSデジピータ・MSGミッションの運用集計）
  - `gyro/`: `gyro_analysis.py`（BOTANジャイロのクォータニオン復元）
- `report/`: 解析結果の報告と図．
  - `operations/`: operation periods, orbit, HK, ground station, images, APRS, gyro
  - `attitude/`: magnetic-alignment analysis, beta angle, settling, spin, alpha/epsilon
  - `gyro/`: BOTAN quaternion reconstruction and scale-factor fix
