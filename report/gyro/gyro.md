...
このセッションのスピンは，キューブのアニメーション（`report/gyro/gyro_20251023_animation.mp4`，実時間）で見ることができる．Z軸まわりの定常回転でスカラー部 qw が周期的にゼロを跨ぐため，生値には17回の符号反転が現れるが，いずれも二重被覆による表示上のもので，符号を連続に補正したキューブは一定速度で回り続ける．3.5秒を超える受信欠落は補間せず，動画上でも飛ばしている．
...
このことは，クォータニオンから姿勢を復元したキューブのアニメーション（`report/gyro/gyro_boot_animation.mp4`）で可視化できる．符号を連続に補正するとキューブは滑らかに回り続け（実姿勢は連続），生値の成分グラフだけが t=30秒で飛ぶ．すなわち不連続は表示上の特異点である．動画はサイズの都合でリポジトリには収録しておらず，`analysis/gyro/gyro_animation.py` で生成する（要ffmpeg）．
