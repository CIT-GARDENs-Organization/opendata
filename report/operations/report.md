...
4衛星はアマチュア無線のAPRSデジピータ（中継器）として運用した．地上の各局が送信したパケットを衛星が中継し，地上局で受信・記録した．受信パケットは `data/downlink/<衛星>/aprs/aprs.csv` に，送信元・宛先・経路・内容を収録した．

| 衛星 | 受信パケット | 局数 | 外部アマチュア局 | 記録日数 |
|---|---:|---:|---:|---:|
| KASHIWA | 6 | 3 | 3 | 1 |
| SAKURA | 318 | 9 | 6 | 9 |
| YOMOGI | 705 | 6 | 4 | 14 |
...
![APRS activity](aprs_activity.png)

パケットの多くはCITの局による動作確認であり，外部アマチュア局の中継は数局にとどまる．SAKURAは外部6局64パケットで最も外部利用が多い．号機別の局一覧は `data/derived/<衛星>/aprs_stations.csv`，集計は `data/derived/00_compare/aprs_summary.csv` に収録した．

SAKURA・YOMOGI・BOTANはメッセージ蓄積（MSG）ミッションも実施し，受信したAPRSメッセージを衛星内メモリに蓄えて後で一括ダウンリンクした．そのダウンリンク16進フレームは `data/downlink/<衛星>/aprs/msg.csv` に，BOTANのデコード済みメッセージは `data/derived/04_botan/aprs_messages.csv`（176件）に収録した．BOTANのメモリには固有メッセージが282件蓄積されていた．

## ジャイロミッション（BOTAN）

BOTANは姿勢センサ（IMU）のクォータニオンをダウンリンクするジャイロミッションを実施した．センサはFSM300（BNO085系9軸IMU，加速度計・ジャイロ・地磁気センサをオンボード融合）である．ダウンリンクフレームは `data/downlink/04_botan/gyro/gyro.csv` に，復元した単位クォータニオンと回転速度は `data/derived/04_botan/gyro_attitude.csv` に収録した．軌道上は自由落下で加速度計の重力基準が失われるが，地磁気センサが絶対方向を与えるためクォータニオンは地磁気を基準とする（周期性が崩れないのはその証拠）．回転（スピン）はジャイロで測れ，求めた回転速度（約15–18°/s）は独立な太陽電池電流の変調（+Z面が定常，側面が周期14–16秒で変調）と整合し，スピン軸が+Z面付近であることを示す．FSM300が基準とする地磁気は，BOTANが姿勢を合わせている当の磁場でもある．詳細と固定小数点スケール（Q14）の誤りは [gyro.md](../gyro/gyro.md) に示す．
