...
BOTANは姿勢センサ（IMU）のクォータニオンをダウンリンクするジャイロミッションを実施した．ダウンリンクフレームは `data/downlink/04_botan/gyro/gyro.csv` に，復元した単位クォータニオンと回転速度は `data/derived/04_botan/gyro_attitude.csv` に収録した．クォータニオンの固定小数点スケール（Q14）と運用時の変換ツールにあったスケール誤り，放出後の回転速度は [gyro.md](../gyro/gyro.md) に示す．
