...
BOTANは姿勢センサ（IMU）のクォータニオンをダウンリンクするジャイロミッションを実施した．ダウンリンクフレームは `data/downlink/04_botan/gyro/gyro.csv` に，復元した単位クォータニオンと回転速度は `data/derived/04_botan/gyro_attitude.csv` に収録した．軌道上は自由落下で加速度計に定常重力がかからないため絶対姿勢は基準を持たないが，回転（スピン）は測れる．クォータニオンから求めた回転速度（約15–18°/s）は，独立な太陽電池電流の変調（+Z面が定常，側面が周期14–16秒で変調）と整合し，スピン軸が+Z面付近であることを示す．固定小数点スケール（Q14）と運用ツールのスケール誤りも含め [gyro.md](../gyro/gyro.md) に示す．
